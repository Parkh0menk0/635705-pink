...
var popup_send_request = document.querySelector(".popup--send-request");
var popup_fail = document.querySelector(".popup--fail");
...
for (var i; i < inputs_required.length; i++) {
  inputs_required[i].oninvalid = function(evt) {
    ...
    popup_fail.classList.add("popup--active");
  }
}
...
if (popup_close) {
  for (var i = 0; i < popup_close.length; i++) {
    popup_close[i].addEventListener("click", function (evt) {
      ...
      popup_fail.classList.remove("popup--active");
